...
service.interceptors.response.use(
  (res) => {
    ...
    const { status ,data} =  res   //后台返回 浏览器的错误码，进行判断
    const { flag, msg, token, code } = res.data
    //更新token保持登录状态
    if (config.isNeedSaveToken) {
      setToken(token)
    }
...
export function axiosReq({
  ...
  return service({
    ...
    isNeedSaveToken:isNeedSaveToken??false   //是否需要 储存 token
  })
}
